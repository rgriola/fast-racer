...
export class Race {
    ...
    private readonly PIXELS_TO_FEET = 0.5; // 1 pixel = 0.5 feet (adjust as needed)
    private readonly FEET_TO_MILES = 1/5280; // Standard conversion

    private readonly DEBUG = true;

    // Track geometry properties
    private readonly trackGeometry: {
        centerX: number;
        centerY: number;
    ...
    private readonly START_LINE = {
        X: 400,
        Y_TOP: 20,
        Y_BOTTOM: 55,
        WIDTH: 3,
        THRESHOLD: 5,
        lastCrossing: new Map<Car, number>()
    };
...
*/
        this.cars.forEach((car, index) => {
            ...
            if (this.DEBUG) {
                console.log(
                    `🚗 ${car.name} at position (${newX.toFixed(1)}, ${newY.toFixed(1)}) ` +
            ...
            if (this.checkStartLineCollision(car, prevPos.x, prevPos.y)) {
                car.lapsCompleted++;
                console.log(
                    `🏁 ${car.name} COMPLETED LAP: ${car.lapsCompleted} ` +

...
function drawWinnerScreen(winner: Car) {
    if (!ctx) return;
...
function drawCarByIndex(index: number) {
    if (!ctx || !cars[index]) return;
...
function generateUniqueNumber(usedNumbers: Set<number>): number {
